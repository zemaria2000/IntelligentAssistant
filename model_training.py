...
if __name__ == '__main__':
    ...
    if args.file is not None:
        ...
        for c in CORR_GROUP[args.model]:
            input_df[c] = scaler.fit_transform(np.array(input_df[c]).reshape(-1,1)) # fit_transform calculates mean and st. deviation for each feature and transforms all points of that feature
            joblib.dump(scaler, f'new_scalers/{c}.scale')   #it persists an arbitrary Python object into one file
            print('for cycle \n\n')
            print(input_df)
    ...
    print('print4')
    rmse = sklearn.metrics.mean_squared_error(test_y, test_predictions, squared=False)
    print(f'RMSE = {rmse}')
