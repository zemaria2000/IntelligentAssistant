    ...
    def add_anomalies(self, anomaly_dataframe: pd.DataFrame):
        ...
        if len(anomaly_dataframe) > 0:
            ...
            for var in anomaly_dataframe.index:
                ...
                if (anomaly_dataframe.loc[var, 'Norm Difference'] >=  anomaly_dataframe.loc[var, 'Thresholds']) and (anomaly_dataframe.loc[var, 'Norm Difference'] < MEDIUM_SEVERITY * anomaly_dataframe.loc[var, 'Thresholds']):
                    severity.append('Light')
                elif (anomaly_dataframe.loc[var, 'Norm Difference'] >= MEDIUM_SEVERITY * anomaly_dataframe.loc[var, 'Thresholds']) and (anomaly_dataframe.loc[var, 'Norm Difference'] < HIGH_SEVERITY * anomaly_dataframe.loc[var, 'Thresholds']):
                    severity.append('Medium')
                elif (anomaly_dataframe.loc[var, 'Norm Difference'] >= HIGH_SEVERITY * anomaly_dataframe.loc[var, 'Thresholds']):
